...
class VarianceAdaptor(nn.Module):
    ...
    def __init__(self, preprocess_config, model_config, train_config, d_model):
        ...
        if self.use_energy_embed:
            ...
            with open(
                os.path.join(preprocess_config["path"]["preprocessed_path"]+f'_{dataset_tag}', "stats.json")
            ) as f:
                stats = json.load(f)
    ...
    def get_pitch_embedding(self, decoder_inp, f0, uv, mel2ph, g_control, l_control=None, encoder_out=None):
        ...
        if self.pitch_type == "ph":
            ...
            # pitch_pred = self.pitch_predictor(pitch_pred_inp) * g_control
            # if l_control is not None:
                # pitch_pred = torch.mul(pitch_pred, l_control.unsqueeze(-1))
            ...
            window_size = max(f0_denorm[0]) - min(f0_denorm[0])
            if g_control is not None:
                f0_denorm = f0_denorm + g_control * window_size / 10
            if l_control is not None:
                f0_denorm = f0_denorm + l_control * window_size / 5
            ...
            if self.pitch_type == "cwt":
                pitch_padding = None
                cwt = cwt_out = self.cwt_predictor(decoder_inp) * g_control
                ...
                pitch_pred = self.pitch_predictor(decoder_inp, f0 if self.training else None) * g_control
                if f0 is None:
                    f0 = pitch_pred[:, :, 0]
            else:
                pitch_pred = self.pitch_predictor(decoder_inp) * g_control
                if f0 is None:
                    f0 = pitch_pred[:, :, 0]
    ...
    def get_energy_embedding(self, x, target, mask, g_control, l_control=None):
        ...
        if target is not None:
            ...
            if g_control is not None:
                prediction = prediction * g_control
            if l_control is not None:
                prediction = torch.mul(prediction, l_control)
    ...
    def forward(
        ...
        if global_control is not None:
            d_control = self.q_func(float(global_control['duration'][0]))
            p_control = float(global_control['pitch'][0])
            e_control = self.q_func(float(global_control['energy'][0]))
        else:
            d_control, p_control, e_control = 1,1,1
        
        if local_control is not None:
            p_local_control, e_local_control, d_local_control = [], [], []
            for i in range(len(local_control)):
                e_local_control.extend([self.q_func(float(local_control['energy'][i]))]*local_control['phone_len'][i])
                d_local_control.extend([self.q_func(float(local_control['duration'][i]))]*local_control['phone_len'][i])
                p_local_control.extend([float(local_control['pitch'][i])]*local_control['phone_len'][i])
            e_local_control = torch.Tensor(e_local_control).unsqueeze(0).to(device)
            d_local_control = torch.Tensor(d_local_control).unsqueeze(0).to(device)
            p_local_control = torch.Tensor(p_local_control).unsqueeze(0).to(device)
        else:
            p_local_control, e_local_control, d_local_control = None, None, None

        x = text.clone()
        if speaker_embedding is not None:
            x = x + speaker_embedding.unsqueeze(1).expand(-1, text.shape[1], -1)
        if emotion_embed_cmt is not None:
            emotion_embed_cmt = torch.from_numpy(emotion_embed_cmt).to(text.device)
            x = x + emotion_embed_cmt.unsqueeze(0).unsqueeze(1).expand(-1, text.shape[1], -1)
        elif emotion_embedding is not None:
            x = x + emotion_embedding.unsqueeze(1).expand(-1, text.shape[1], -1)
        
        if intensity_embed_cmt is not None:
            intensity_embed_cmt = torch.from_numpy(intensity_embed_cmt).to(text.device)
            x = x + intensity_embed_cmt.unsqueeze(0).unsqueeze(1).expand(-1, text.shape[1], -1)
        elif intensity_embedding is not None:
            x = x + intensity_embedding.unsqueeze(1).expand(-1, text.shape[1], -1)
        ...
        if attn_prior is not None: # Trainig of unsupervised duration modeling
            ...
            duration_meta = (torch.exp(log_duration_prediction) - 1)
            if global_control is not None:
                duration_meta = torch.mul(duration_meta, d_control)
            if local_control is not None:
                duration_meta = torch.mul(duration_meta, d_local_control)
            duration_rounded = torch.clamp(
                torch.round(duration_meta),
                min=0,
            )
        ...
        if self.use_pitch_embed:
            if pitch_target is not None:
                ...
                if local_control is not None:
                    pitch_prediction, pitch_embedding = self.get_pitch_embedding(
                        x, None, None, mel2ph, p_control, p_local_control, encoder_out=x_org
                    )
                else:
                    pitch_prediction, pitch_embedding = self.get_pitch_embedding(
        ...
        if self.use_energy_embed and self.energy_feature_level == "frame_level":
            ...
            if local_control is not None:    
                energy_prediction, energy_embedding = self.get_energy_embedding(x_org, energy_target, src_mask, e_control, e_local_control)
            else:
