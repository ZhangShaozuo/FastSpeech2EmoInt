...
class CompTransTTSLossOutput(ModelOutput):
    ...
    bin_loss: Optional[torch.FloatTensor] = None
    prosody_loss: Optional[torch.FloatTensor] = None
...
class CompTransTTSLoss(nn.Module):
    ...
    def forward(self, inputs, predictions, step):
        '''From the Inputs'''
        texts = inputs['text']
        mel_targets = inputs['mel']
        pitch_targets = inputs['pitch_data']
        energy_targets = inputs['energy']
        duration_targets = inputs['duration']
        ...
        src_lens = predictions.src_lens
        mel_lens = predictions.mel_lens
        attn_outs = predictions.attn_outs
        prosody_info = predictions.prosody_info ### Default setting is None
        ...
        emotion_loss = None
        intensity_loss = None
        if predictions.emotion_outputs is not None:
            emotion_loss = predictions.emotion_outputs.loss
            total_loss += emotion_loss
        if predictions.intensity_outputs is not None:
            intensity_loss = predictions.intensity_outputs.loss
            total_loss += intensity_loss
        
        return CompTransTTSLossOutput(
